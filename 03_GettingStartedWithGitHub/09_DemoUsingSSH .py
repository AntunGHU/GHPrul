...
# na zalost key-si su se kreirali u mapi "aCod" a dobili su ime mog zadnjeg unosa u path kad sam odgovarao na lokaciju sa "/home/antun/aCod/vGitVsc". Ok, nesto smo naucili nova!!! NOVO!!!
# otvaramo pub.key i nagalasavamo da je to key koji treba transferati na GH. Kopira pub-key i ide na GH -dashboard, profile i Settings i SSH pa klik na "NewSSH keys" (kod mene je sve to prazno sto znaci da bi ja ipak mogao unijeti svoj ssh-pub-key, kopiram i unosim te add-am!) Pokazuje se key i upozorenje da obrisem bilo koji koji ne prepoznajem!
